...
class KGEModel(nn.Module):
    ...
    def forward(self, sample, mode='single'):
        ...
        if mode == 'single' and self.model_name != 'MDE':
            ...
            #print(h_d.is_cuda)
            a = self.node_features[0]#.cuda()
            t_d = torch.index_select(a, dim=0, index=tail_part.view(-1)).view(batch_size, negative_sample_size)
    ...
    def MDE(self, h, r, t, mode):
        ...
        r_deg = torch.abs((torch.cos(h[9]) - torch.cos(t[9])))#, dtype=torch.float, device=device)
        #print(r_deg.is_cuda)
        #print(r_deg.shape)
        ...
        # c = t + r - h
        # d = h - r * t
        score_j = torch.norm((h[8] - t[8]), p=2, dim=2)
        #print(score_j.is_cuda)
        #print(score_j.shape)
        #print("r_deg in forward:", r_deg)
        score_j = score_j  - r_deg
        ...
        score_all = (score_a +score_j)/2.0#(2 * score_a + 4.0 * score_b + 2 * score_c + 4.0 * score_d + 2 * score_j) / 14.0
