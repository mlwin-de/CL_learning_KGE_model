...
#from model_ablation_with_g_score_only import KGEModel
from model_ablation_with_g_score_and_first_term import KGEModel
#from model_ablation_with_out_g import  KGEModel
...
def get_node_features(args, entity2id):
    ...
    _pagerank[0:_pagerank_.shape[0]] = array_norm(torch.tensor(_pagerank_))
    ...
    _centrality[0:_centrality_.shape[0]] = array_norm(torch.tensor(_centrality_))
    ...
    _betweenness[0:_betweenness_.shape[0]] = array_norm(torch.tensor(_betweenness_))
    ...
    _katz[0:_katz_.shape[0]] = array_norm(torch.tensor(_katz_))
...
def main(args):
    ...
    if args.triples_are_mapped:
        ...
        valid_triples, entity2id_new, relation2id_new = read_mapped_triples(os.path.join(args.data_path, 'valid.txt'),args)
        logging.info('#valid: %d' % len(valid_triples))
        test_triples, entity2id_new, relation2id_new = read_mapped_triples(os.path.join(args.data_path, 'test.txt'),args)
    ...
    #n_f = NodeFeatures()
    node_features = get_node_features(args, entity2id) #node_features =
    ...
    if args.do_train:
        ...
        for step in range(init_step, args.max_steps):
            ...
            if args.do_test and step > 90000 and step % args.valid_steps == 0:
