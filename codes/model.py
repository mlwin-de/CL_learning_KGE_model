...
class KGEModel(nn.Module):
    def __init__(self, model_name, nentity, nrelation, hidden_dim, gamma,
        ...
        if model_name == 'MDE':
            self.entity_embedding = nn.Parameter(torch.zeros(nentity, self.entity_dim,device=device))
            self.entity_embedding2 = nn.Parameter(torch.zeros(nentity, self.entity_dim,device=device))
            self.entity_embedding3 = nn.Parameter(torch.zeros(nentity, self.entity_dim,device=device))
            ...
            self.entity_embedding7 = nn.Parameter(torch.zeros(nentity, self.entity_dim,device=device))
            ...
            self.relation_embedding = nn.Parameter(torch.zeros(nrelation, self.relation_dim,device=device))
            self.relation_embedding2 = nn.Parameter(torch.zeros(nrelation, self.relation_dim,device=device))
            ...
            self.relation_embedding5 = nn.Parameter(torch.zeros(nrelation, self.relation_dim,device=device))
            self.relation_embedding6 = nn.Parameter(torch.zeros(nrelation, self.relation_dim,device=device))
            self.relation_embedding7 = nn.Parameter(torch.zeros(nrelation, self.relation_dim,device=device))
            self.relation_embedding8 = nn.Parameter(torch.zeros(nrelation, self.relation_dim,device=device))
            nn.init.uniform_(
                tensor=self.relation_embedding,
                a=-self.embedding_range.item(),
                b=self.embedding_range.item()
    ...
    def set_node_features(self, args, node_features):
        ...
        if args.cuda:
            self.node_features = node_features.cuda()
    ...
    def forward(self, sample, mode='single'):
        ...
        if mode == 'single' and self.model_name != 'MDE':
            ...
            for i in range(0, 6):
                ...
                t_d.append(t_d_)
            #for i in range(0,6):
            #    h_d[i]=self.node_features[i][sample[:, 0]].view(batch_size, -1)
            #    t_d[i] = self.node_features[i][sample[:, 2]].view(batch_size, -1)

            head = [h, h2, h3, h4, h5,h6,h7,h8,h9,h_d]
            relation = [r, r2, r3, r4,r5,r6,r7,r8]
            tail = [t, t2, t3, t4, t5,t6,t7,t8, t9,t_d]
            ...
            t_d = []  # self.zero_f_tensor.clone()#torch.zeros(6)
            for i in range(0, 6):
                h_d_ = torch.index_select(self.node_features[i], dim=0, index=head_part.view(-1)).view(batch_size, negative_sample_size)
            ...
            for i in range(0, 6):
                ...
                t_d_ = torch.index_select(self.node_features[i], dim=0, index=tail_part.view(-1)).view(batch_size, negative_sample_size)
                h_d.append(h_d_)
                t_d.append(t_d_)
    ...
    def MDE(self, h, r, t, mode):
        ...
        # c = t + r - h
        # d = h - r * t
        r_f_0 = torch.abs((torch.cos(h[9][0]) - torch.cos(t[9][0])))#, dtype=torch.float, device=device)
        r_f_1 = torch.abs((torch.cos(h[9][1]) - torch.cos(t[9][1])))
        r_f_2 = torch.abs((torch.cos(h[9][2]) - torch.cos(t[9][2])))
        r_f_3 = torch.abs((torch.cos(h[9][3]) - torch.cos(t[9][3])))
        r_f_4 = torch.abs((torch.cos(h[9][4]) - torch.cos(t[9][4])))
        r_f_5 = torch.abs((torch.cos(h[9][5]) - torch.cos(t[9][5])))
        #print(r_deg.is_cuda)
        #print(r_deg.shape)
        score_f_0 = torch.norm((h[8] - t[8]), p=2, dim=2)
        #print(score_j.is_cuda)
        #print(score_j.shape)
        #print("r_deg in forward:", r_deg)
        score_f_1 = score_f_0  - r_f_0 -r_f_1 -r_f_2 -r_f_3 -r_f_4 - r_f_5
        #print("printing score_j in the mde_g forward:",score_j)
        if mode == 'head-batch':
            a = h[0] + (r[0] - t[0])
            b = h[1] + (t[1] - r[1])
            c = t[2] + (r[2] - h[2])
        ...
        score_all = (2 * score_a + 4.0 * score_b + 2 * score_c + 4.0 * score_d + 2 * score_f_1) / 14.0
        ...
        #print("min max of score in forward:", torch.min(score), torch.max(score))
        return score
    ...
    # def get_node_features(self, args, entity2id):
    #     _degree = torch.ones(len(entity2id)) * 10
    #     _degree_ = np.load(args.node_feat_path + "/degree.npy")
    #     max_dg = np.log(np.max(_degree_[:, 1]))
    #     _degree[0:_degree_.shape[0]] = torch.log(torch.tensor(_degree_[:, 1], dtype=float)) / max_dg
    #
    ...
    #     _pagerank[0:_pagerank_.shape[0]] = self.array_norm(torch.tensor(_pagerank_))
    #
    ...
    #     _centrality[0:_centrality_.shape[0]] = self.array_norm(torch.tensor(_centrality_))
    #
    ...
    #     _closeness[0:_closeness_.shape[0]] = self.array_norm(torch.tensor(_closeness_))
    #
    ...
    #     _betweenness[0:_betweenness_.shape[0]] = self.array_norm(torch.tensor(_betweenness_))
    #
    ...
    #     _katz[0:_katz_.shape[0]] = self.array_norm(torch.tensor(_katz_))
    #
    #     return torch.stack((_degree,_pagerank,_centrality,_closeness,_betweenness,_katz),dim=1).t()
    #
